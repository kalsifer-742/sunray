...
use ash::{khr::acceleration_structure, vk::{AccelerationStructureBuildGeometryInfoKHR, AccelerationStructureBuildRangeInfoKHR, AccelerationStructureBuildSizesInfoKHR, AccelerationStructureBuildTypeKHR, AccelerationStructureCreateInfoKHR, AccelerationStructureGeometryDataKHR, AccelerationStructureGeometryKHR, AccelerationStructureGeometryTrianglesDataKHR, AccelerationStructureKHR, AccelerationStructureTypeKHR, BufferUsageFlags, BuildAccelerationStructureFlagsKHR, BuildAccelerationStructureModeKHR, CommandBufferBeginInfo, CommandBufferUsageFlags, DeviceOrHostAddressConstKHR, DeviceOrHostAddressKHR, Format, GeometryFlagsKHR, GeometryTypeKHR, MemoryAllocateFlags, MemoryPropertyFlags, PhysicalDeviceMemoryProperties}, Device};
...
impl BLAS {
    pub fn new(
        ...
        // specify the range of values in the ibo to be read and a "transform offset" (?).
        // multiple 'build_range_info's may be specified one for each geometry, I assume
        // this is used for multiple instances (?)
        let build_range_info = AccelerationStructureBuildRangeInfoKHR::default()
            .first_vertex(0)
        ...
        // with some additional args based on the allocations that were done.
        ...
        unsafe {
            ...
            device.end_command_buffer(build_command_buffer).to_sr_result()?
        }
        ...
        Ok(Self { blas, blas_buffer })
    }

    #[allow(dead_code)]
    pub fn buffer(&self) -> &vulkan_abstraction::Buffer { &self.blas_buffer }
}
impl Drop for BLAS {
    fn drop(&mut self) {
    }
...
impl Deref for BLAS {
    ...
    fn deref(&self) -> &Self::Target { &self.blas }
}