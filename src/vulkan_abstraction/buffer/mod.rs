...
use std::ops::Deref;
use ash::{vk, Device};
use ash::vk::{BufferUsageFlags, CommandPool, DeviceMemory, DeviceSize, MemoryAllocateFlags, MemoryAllocateFlagsInfo, MemoryPropertyFlags, PhysicalDeviceMemoryProperties};
use crate::error::*;
...
impl Buffer {
    pub fn new_staging<V>(device: Device, size: usize, mem_props : &PhysicalDeviceMemoryProperties) -> SrResult<Self> {
        ...
        Self::new::<V>(device, size, mem_flags, mem_alloc_flags, usage_flags, mem_props)
    }
    
    #[allow(dead_code)]
    pub fn new_uniform<T>(device: Device, mem_props : &PhysicalDeviceMemoryProperties) -> SrResult<Self> {
        ...
        Self::new::<T>(device, 1, mem_flags, mem_alloc_flags, usage_flags, mem_props)
    }

    pub fn new_staging_from_data<T: Copy>(device: Device, data: &[T], mem_props : &PhysicalDeviceMemoryProperties) -> SrResult<Self> {
    ...
    pub fn new<V>(
        device: Device, len: usize, mem_flags: MemoryPropertyFlags, alloc_flags: MemoryAllocateFlags,
        usage_flags: BufferUsageFlags, mem_props : &PhysicalDeviceMemoryProperties
    ) -> SrResult<Self> {
        ...
        let mut mem_alloc_flags_info = MemoryAllocateFlagsInfo::default()
            .flags(alloc_flags);
        ...
        let memory = unsafe { device.allocate_memory(&mem_alloc_info, None)  }.to_sr_result()?;
    ...
    pub fn map<V>(&mut self) -> SrResult<&mut [V]> {
        ...
        let p = unsafe { self.device.map_memory(self.memory, 0, self.usable_byte_size, flags)  }.to_sr_result()?;
    ...
    // mut borrow of self was already dropped
    ...
    //mainly useful to copy from a staging buffer to a device local buffer
    pub fn clone_buffer(device: &Device, queue: &Queue, cmd_pool: &CommandPool, src: &Buffer, dst: &Buffer) -> SrResult<()>{
        ...
        unsafe { device.begin_command_buffer(bufcpy_cmd_buf, &begin_info)  }.to_sr_result()?;
    ...
    pub fn get_device_address(&self) -> vk::DeviceAddress {
        let buffer_device_address_info = vk::BufferDeviceAddressInfo::default()
            .buffer(self.buffer);
        unsafe {
            self.device.get_buffer_device_address(&buffer_device_address_info)
        }
    }
}
