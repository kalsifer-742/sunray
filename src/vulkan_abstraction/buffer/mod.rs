...
impl Buffer {
    ...
    pub fn new<V>(
        ...
        let memory = unsafe { device.allocate_memory(&mem_alloc_info, None)  }.map_err(SrError::from)?;
    ...
    pub fn map<V>(&mut self) -> SrResult<&mut [V]> {
        ...
        let p = unsafe { self.device.map_memory(self.memory, 0, self.usable_byte_size, flags)  }.map_err(SrError::from)?;
    ...
    pub fn clone_buffer(device: &Device, queue: &Queue, cmd_pool: &CommandPool, src: &Buffer, dst: &Buffer) -> SrResult<()>{
        ...
        unsafe { device.begin_command_buffer(bufcpy_cmd_buf, &begin_info)  }.map_err(SrError::from)?;
