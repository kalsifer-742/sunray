...
impl Queue {
    pub fn new(device: Device, swapchain_device: khr::swapchain::Device, q_family: u32, q_index: u32) -> SrResult<Self> {
        ...
        let create_semaphore =  || unsafe_vk!{ device.create_semaphore(&vk::SemaphoreCreateInfo::default(), None) };
        ...
        let create_fence = || {
            ...
            unsafe_vk!{ device.create_fence(&fence_info, None) }
        };
    ...
    pub fn wait_idle(&self) -> SrResult<()> {
        unsafe_vk!{ self.device.queue_wait_idle(self.queue) }?;
        Ok(())
    }
    ...
    pub fn acquire_next_image(&self, swapchain: vk::SwapchainKHR) -> SrResult<u32> {
        ...
        unsafe_vk!{ self.device.wait_for_fences(wait_fence, true, u64::MAX) }?;
        unsafe_vk!{ self.device.reset_fences(wait_fence) }?;
        

        let image_available_sem = self.img_available_sem[self.current_frame];
        let (index, _suboptimal_surface) = unsafe_vk!{
    ...
    pub fn submit_async(&self, command_buffer: vk::CommandBuffer) -> SrResult<()> {
        ...
        unsafe_vk!{ self.device.queue_submit(self.queue, &[submit_info], signal_fence) }?;
    ...
    pub fn submit_sync(&self, command_buffer: vk::CommandBuffer) -> SrResult<()> {
        ...
        unsafe_vk!{ self.device.queue_submit(self.queue, &[submit_info], Fence::null()) }?;
        Ok(())
    }
    ...
    pub fn present(&mut self, swapchain: vk::SwapchainKHR, img_idx: u32) -> SrResult<()> {
        ...
        unsafe_vk!{ self.swapchain_device.queue_present(self.queue, &present_info) }?;
