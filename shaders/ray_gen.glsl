...
void main() {
    ...
    vec4 origin    = matrices_uniform_buffer.view_inverse * vec4(0, 0, 0, 1); //transform to world_space
    vec4 target    = matrices_uniform_buffer.proj_inverse * vec4(d.x, d.y, 1, 1); //transform to view_space
    vec4 direction = matrices_uniform_buffer.view_inverse * vec4(normalize(target.xyz), 0); //transform to world_space
